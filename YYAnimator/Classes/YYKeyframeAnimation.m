...
@implementation YYKeyframeAnimation

+ (instancetype)animationWithKeyPath:(NSString *)path {
    YYKeyframeAnimation *animation = [super animationWithKeyPath:path];
    animation.functionBlock = ^double(double t, double b, double c, double d) {
        return YYKeyframeAnimationFunctionLinear(t, b, c, d);
    };
...
- (id)copyWithZone:(nullable NSZone *)zone {
    YYKeyframeAnimation *copy = [[YYKeyframeAnimation allocWithZone:zone] init];
    ...
    if (self.customizedValues) {
        copy.customizedValues = [self.customizedValues copy];
    }
    
    if (self.customizedKeyTimes) {
        copy.customizedKeyTimes = [self.customizedKeyTimes copy];
    }
...
- (NSArray *)createRectArrayFromXValues:(NSArray *)xValues
    ...
    NSAssert(xValues.count == yValues.count && xValues.count == widths.count && xValues.count == heights.count, @"array must have arraYY of equal size");
...
- (NSArray *)createPointArrayFromXValues:(NSArray *)xValues yValues:(NSArray *)yValues {
    NSAssert(xValues.count == yValues.count, @"array must have arraYY of equal size");
...
- (NSArray *)createSizeArrayFromWidths:(NSArray *)widths heights:(NSArray *)heights {
    NSAssert(widths.count == heights.count, @"array must have arraYY of equal size");
...
- (NSArray *)createColorArrayFromRed:(NSArray*)redValues
    ...
    NSAssert(redValues.count == blueValues.count && redValues.count == greenValues.count && redValues.count == alphaValues.count, @"arraYY must have arraYY of equal size");
